...
def load_spectrum_image(file_name):
    ...
    Returns:
    --------
    Hyoerspy spectrum image of shape (x_lenght,y_lenght,n_channels).
    '''
...
def kmeans_clustering(spectra,n_clusters,pca=False,n_comp=8):
    '''
    This function applies clustering via kmeans to Digital Micrograph spectrum_images. 
    
    -spectra: hyperspy spectrum images of shape (x_lenght,y_lenght,n_channels)
    -n_clusters = int. 
        Number of clusters imposed in the kmeans clustering routine.
    -pca = boolean. (default = False)
        True, it applies PCA to clean the spectra.
        False, it doesn't apply the PCA clean. 
    -n_comp = int. (default = 10)
        Number of principal components selected from the PCA routine.

    Returns:
    --------
    labels: labels of each spectrum in the spectrum image. 
    centres: centroids of each cluster identified in the spectrum image. 
    '''
    if pca:
...
def preprocesing_spectrum_image(spectrum_image,labels,indx_lbl,ax,bx,a_bck,b_bck,pca=False,n_comp=8):
    '''
    This function extract an array of sepctra from an initial spectrum images which has been identified by 
    the kmeans clustering. You have to provide the value of the cluster in which your are interested to get the spectra. 

    spectrum_image: hyperspy spectrum image of shape (x_lenght,y_lenght,n_channels).
        Spectrun images. 
    labels: array like of shape (x_lenght,y_lenght).
        Labels of the kemans clustering. 
    indx_lbl: int.
    ...
    a_bck: float.
        Initial energy value used to apply a Powerlaw fit and remove the backgroung. 
    b_bck: float.
        Ending energy value used to apply a Powerlaw fit and remove the backgroung. 
    pca: boolean (default = False). 
        True, it applies PCA to clean the spectra.
        False, it doesn't apply the PCA clean. 
    ...
    end_spectra: array-like of shape (n_spectra,r_channels)
        n_spectra is the number of interesting spectra resulting from the spectrum image. 
...
def apply_mask(spectrum_image,labels,mask_label):
    '''
    This functions applies a mask under the mask_label values. 
    ...
    Returns:
    --------
    Returns an spectrum images with 0 at the masked regions. 
    '''
...
def plot_clustering(labels,centroids,cm,pix_size=1e-9,fig_save=None):
    '''
    This function plots in a colormap the results obtained by clustering. 
    labels: array-like.
        Labels for each spectrum of an spectrum image. 
    centroids: array-like.
        Centroid of each cluster.
    cm: ListedColormap.
        List of colors of the colormap. 
        i.e. cm = mpl.colors.ListedColormap(['black','red','green'])
    pix_size: float. (default = 1nm)
        Spatial resolution in meters of the image. 

    Return:
    -------
    It returns a color map. 
    '''
...
def crop_spectra(spectra,min,max):
    ...
    max: float.
        Maximum value to crop the sepctra. 
